...
//    - char 형은 2바이트 정수를 저장하여 문자(모양)를 표현한다.
	public static void main(String[] args) {
		...
		char[] carr2 = new char[20]; // 기본초기값은 0
		System.out.println("[[[ char 배열 테스트 - 1 ]]]");
		System.out.println("carr1 : " + Arrays.toString(carr1));
		System.out.println("carr2 : " + Arrays.toString(carr2)); // 값 0에대한 문자는 빈칸
...
//		carr2 = "hello hi goodbye";  char배열에 바로 문자열 대입은 오류
		carr2 = "hello hi goodbye".toCharArray(); // String 문자열 객체를 char 배열로 변환(not static)
		System.out.println(carr2);
		System.out.println(carr2[2]);
		System.out.println("carr2.length : " + carr2.length); 
		
		carr2 = "hello hi goodbye hi goodbye".toCharArray(); // 문자열이 선언된 배열크기보다 클때
		System.out.println(carr2);
		System.out.println(carr2.length); // 27  >> 배열의 크기가 커진다.
		
		String temp = String.valueOf(carr2);  // char[]배열을 문자열로 변환
											  // String 객체의 toCharArray()메소드와 반대
		System.out.println(temp);
		
		// 참고 : String.valueOf 메소드는 여러가지 매개변수 형식을 다룬다.(메소드 오버로딩)

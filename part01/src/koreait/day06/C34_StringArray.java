...
//		String message = "hi java!";
		...
		String[] koreans = new String[5];   // 배열요소의 값이 기본형 데이터 저장X, 참조값을 저장
...
//		koreans[4] = "동적인";
		...
		for(int i=0; i<koreans.length; i++)
			if(koreans[i] != null) // 중요!!! ************ 이문장을 넣어주면 null값이 다 안만들어져있어도 오류가 안난다.
			System.out.println(koreans[i] + ":" + koreans[i].length());
		// 참조하고있는 객체가 없을 때 메소드/필드 참조하면 오류
...
//		System.out.println(temp.length());    오류
		...
		// break 로 for를 중단하지 않고 for를 모두 실행했을때는 같은 문자열이 배열 engKey에 없을때
		System.out.println("찾은 단어 위치 : " + i);
		if(i == engKey.length)
			System.out.println("찾는단어가 engKey 에 없습니다!");
		else
